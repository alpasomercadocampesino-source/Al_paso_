...
export default function AdminDashboard({ adminName, lastGlobalSync, sucursalAsignada }: AdminDashboardProps) {
  ...
  // ── Cambios de precio ───────────────────────────────────────────────────
  // La bitácora guarda una fila por producto, pero los precios se cambian de
  // tanda: al guardar la matriz se escriben de golpe todos los productos
  // editados. La tarjeta mostraba el total de filas, así que una sola tanda de
  // 106 productos se leía como si se hubiera cambiado todo hoy.
  //
  // Se agrupa por minuto: una tanda entra en milisegundos, así que cada minuto
  // con filas es una actualización.
  const tandasDePrecio = priceHistory.reduce((mapa, h) => {
    const minuto = (h?.Fecha_Hora || "").slice(0, 16);
    if (!minuto) return mapa;
    mapa.set(minuto, (mapa.get(minuto) || 0) + 1);
    return mapa;
  }, new Map<string, number>());
  const minutosOrdenados = [...tandasDePrecio.keys()].sort();
  const ultimaTanda = minutosOrdenados[minutosOrdenados.length - 1] || "";
  const productosUltimaTanda = tandasDePrecio.get(ultimaTanda) || 0;
        ...
        {adminMode === "master" && (
          <div className="max-w-7xl mx-auto px-4 md:px-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col justify-between">
                <div>
                  <span className="text-slate-400 text-xs font-semibold uppercase tracking-wider block">Última Actualización de Precios</span>
                  <h3 className="text-3xl font-extrabold text-slate-800 mt-2">
                    {productosUltimaTanda} {productosUltimaTanda === 1 ? "Producto" : "Productos"}
                  </h3>
                  ...
                  <p className="text-slate-400 text-[11px] mt-1.5">
                    {minutosOrdenados.length} {minutosOrdenados.length === 1 ? "actualización" : "actualizaciones"} en total
                    {" · "}{priceHistory.length} cambios de precio en la bitácora
                  </p>
                </div>
                <button
                  onClick={() => setAdminMode("history")}
